...
function hasPermission(menus, route) {
    if (route.menu) {
        /**
         * 如果这个路由有 menu 属性，就需要判断此用户是否拥有此 menu 权限
         */
...
function filterAsyncRouter(asyncRouterMap, menus) {
    const accessedRouters = asyncRouterMap.filter(route => {
        // filter, js 语法里面数组的过滤筛选方法
        if (hasPermission(menus, route)) {
            if (route.children && route.children.length) {
                // 如果这个路由下面还有下一级的话，就递归调用
                route.children = filterAsyncRouter(route.children, menus);
                // 如果过滤一遍后，没有子元素了，这个父级菜单就也不显示了
...
const permission = {
    state: {
        routers: constantRouterMap,   // 本用户所有的路由，包括了固定的路由和下面的 addRouters
    ...
    mutations: {
        SET_ROUTERS: (state, routers) => {
            ...
            state.routes = constantRouterMap.concat(routers); // 将固定路由和新增路由进行合并，成为本用户的最终的路由信息
        }
    },
